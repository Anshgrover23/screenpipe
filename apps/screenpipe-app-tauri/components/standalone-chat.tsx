...
export function StandaloneChat({
  ...
  const pendingArtifactPreviewRef = useRef<{
    sessionId: string;
    path: string;
  } | null>(null);

  useEffect(() => {
    const pending = pendingArtifactPreviewRef.current;
    if (!pending || !conversationId || pending.sessionId !== conversationId) {
      return;
    }
    pendingArtifactPreviewRef.current = null;
    openFilePreview(pending.path);
  }, [conversationId, openFilePreview]);
  ...
  const openConversationLocally = useCallback(async (convId: string) => {
    ...
    if (session?.messages && session.messages.length > 0) {
      ...
      await loadConversationRef.current({
        ...
        ...(session.pipeContext ? { pipeContext: session.pipeContext } : {}),
      } as any);
      return;
    }
  ...
  useEffect(() => {
    const artifactHash = (value: string) => {
      let hash = 0;
    ...
    const latestPipeSessionForArtifact = async (pipeName: string) => {
      const storeSessions = Object.values(useChatStore.getState().sessions);
      const fromStore = storeSessions
        .filter(
          (session) =>
            (session.kind === "pipe-run" || session.kind === "pipe-watch") &&
            session.pipeContext?.pipeName === pipeName,
        )
        .sort((a, b) => {
          const aTime = a.lastContentAt ?? a.updatedAt ?? a.createdAt;
          const bTime = b.lastContentAt ?? b.updatedAt ?? b.createdAt;
          return bTime - aTime;
        })[0];
      if (fromStore) return fromStore.id;

      const { loadAllConversations } = await import("@/lib/chat-storage");
      const fromDisk = (await loadAllConversations({
        kind: "pipe-run",
        includeHidden: true,
      }))
        .filter((conv) => conv.pipeContext?.pipeName === pipeName)
        .sort((a, b) => {
          const aTime = a.lastContentAt ?? a.updatedAt ?? a.createdAt;
          const bTime = b.lastContentAt ?? b.updatedAt ?? b.createdAt;
          return bTime - aTime;
        })[0];
      return fromDisk?.id ?? null;
    };

    const createRecoveredArtifactConversation = async (detail: any) => {
      const source = typeof detail.source === "string" ? detail.source : "pipe";
      const path = typeof detail.path === "string" ? detail.path : "";
      const title = typeof detail.title === "string" ? detail.title : "artifact";
      const explicitId =
        typeof detail.id === "number" || typeof detail.id === "string"
          ? String(detail.id)
          : artifactHash(path || title);
      const sessionId = `artifact:pipe:${source}:${explicitId}`;
      const modifiedAt =
        typeof detail.modifiedAt === "string" ? Date.parse(detail.modifiedAt) : NaN;
      const createdAt =
        typeof detail.createdAt === "string" ? Date.parse(detail.createdAt) : NaN;
      const now = Date.now();
      const timestamp = Number.isFinite(modifiedAt)
        ? modifiedAt
        ...
        messages: [
          {
            ...
            role: "assistant",
            content:
              `artifact "${title}" was generated by pipe ${source}.\n\n` +
              "the original pipe-run chat was not found in local history, so this recovered context was created for the artifact preview.",
            timestamp,
          },
        ],
        createdAt: timestamp,
      ...
      const { saveConversationFile, conversationMetaFromJson } = await import(
        "@/lib/chat-storage"
      );
      const { sessionRecordFromMeta } = await import("@/lib/stores/chat-store");
    ...
    const openPreviewAfterConversationSwitch = (
      sessionId: string,
      path: string,
    ) => {
      ...
      pendingArtifactPreviewRef.current = { sessionId, path };
    };
    ...
    const handler = async (event: Event) => {
      ...
      const sourceType =
        typeof detail.sourceType === "string" ? detail.sourceType : null;
      const source = typeof detail.source === "string" ? detail.source : null;

      let targetSessionId: string | null = null;
      if (sourceType === "chat" && source) {
        targetSessionId = source;
        await openConversationLocally(source);
      } else if (sourceType === "pipe" && source) {
        const pipeSessionId = await latestPipeSessionForArtifact(source);
        if (pipeSessionId) {
          targetSessionId = pipeSessionId;
          await openConversationLocally(pipeSessionId);
        } else {
          targetSessionId = await createRecoveredArtifactConversation(detail);
          await openConversationLocally(targetSessionId);
        }
      } else {
      ...
      if (targetSessionId) {
        openPreviewAfterConversationSwitch(targetSessionId, path);
      } else {
        openFilePreview(path);
      }
    };

    window.addEventListener("open-artifact-workspace", handler);
    return () => window.removeEventListener("open-artifact-workspace", handler);
  }, [conversationId, openConversationLocally, openFilePreview]);
